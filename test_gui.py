# GUI.py
import pygame
from sudoku_solver import solve_board, check_valid
import time
pygame.font.init()
...
class Cube:
    ...
    def draw(self, win):
        fnt1 = pygame.font.SysFont("timesnewroman", 45)
        fnt2 = pygame.font.SysFont("timesnewroman", 20)
        gap = self.width / 9
        x = self.col * gap
        y = self.row * gap
        #
        if self.temp != 0 and self.value == 0:
            text = fnt2.render(str(self.temp), 1, (128, 128, 128))
            win.blit(text, (x+50, y+5))
        elif not (self.value == 0):
            text = fnt1.render(str(self.value), 1, (0, 0, 0))
